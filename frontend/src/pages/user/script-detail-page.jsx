...
import { ArrowLeft, FileText, CalendarDays, Pencil } from "lucide-react";
...
export default function ScriptDetailPage() {
  ...
  const [form, setForm] = useState({ judulScript: "", script: "", revisionNote: "" });
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const json = await api(`/api/me/scripts/${id}`);
        setData(json.data);
      } catch (e) {
        setError(e.message);
      } finally {
  ...
  const startEdit = () => {
    setForm({ judulScript: data.judulScript || "", script: data.script || "", revisionNote: data.revisionNote || "" });
    setSaveError("");
    setEditing(true);
  };

  const saveEdit = async () => {
    if (!form.judulScript.trim()) {
      setSaveError("Script title is required");
      return;
    }
    setSaving(true);
    setSaveError("");
    try {
      const json = await api(`/api/me/scripts/${id}`, {
        method: "PUT",
        body: JSON.stringify({
          judulScript: form.judulScript.trim(),
          script: form.script,
      ...
      setData(json.data);
      setEditing(false);
      setNotice("Script updated and reset to pending for review.");
    } catch (e) {
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
          <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-800 flex items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-amber-500/10 text-amber-500 shrink-0">
              ...
              <div>
                {editing ? (
                  <input
                    type="text"
                    value={form.judulScript}
                    onChange={(e) => setForm({ ...form, judulScript: e.target.value })}
                    className="w-full max-w-md px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-lg font-bold text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500/50"
                  />
                ) : (
                  <h1 className="text-lg font-bold text-gray-900 dark:text-white">{data.judulScript}</h1>
                )}
            ...
            {editing ? (
              <>
                <textarea
                  ...
                  rows={10}
                  className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-red-500/50"
                ...
                <div className="mt-4 flex items-center gap-3">
                  <button
                    onClick={saveEdit}
                    disabled={saving}
                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-red-500 hover:bg-red-600 text-white disabled:opacity-50 transition-colors"
                  >
              ...
              <p className="text-sm text-gray-400 italic">No script content.</p>
            )}
            {!editing && data.fileScript && (
              <a
                href={data.fileScript}
                target="_blank"
                rel="noreferrer"
                className="mt-4 inline-flex items-center gap-1.5 text-sm text-blue-500 hover:underline"
              >
                <CalendarDays className="h-4 w-4" /> Open attached file
              </a>
            )}
