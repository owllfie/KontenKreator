...
export default function BackupPage() {
  ...
  const handleBulkDelete = async () => {
    setBulkLoading(true);
    try {
      const res = await fetch(`${API}/api/admin/backup/${bulkDialog.type === "all" ? "all" : "bulk"}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getToken()}`,
        },
        body: bulkDialog.type === "all" ? undefined : JSON.stringify({ ids: Array.from(selected).map(Number) }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        showNotice(data.message || "Failed to delete backups", "error");
        return;
      }
      setBulkDialog({ open: false, type: "selected" });
      setSelected(new Set());
      showNotice(data.message || "Backups deleted successfully");
      await refresh();
    } catch {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showNotice(data.message || "Failed to create backup", "error");
        return;
      }
      await refresh();
      showNotice(`Backup created successfully: ${data.fileName}`);
    } catch {
      showNotice("Cannot connect to the server", "error");
    } finally {
  ...
  const handleDownload = async (b) => {
    try {
      ...
      if (!res.ok) {
        showNotice("Failed to download backup", "error");
        return;
      }
  ...
  const handleRestore = async (b) => {
    if (!window.confirm(`Restore database from "${b.fileName}"? Current data will be overwritten.`)) return;
    setBusyId(b.id);
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        showNotice(data.message || "Failed to restore database", "error");
        return;
      }
      showNotice(data.message || "Database restored successfully");
    } catch {
      showNotice("Cannot connect to the server", "error");
    } finally {
      setBusyId(null);
  ...
  const handleDelete = async (b) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        showNotice(data.message || "Failed to delete backup", "error");
        return;
      }
      await refresh();
      showNotice("Backup deleted");
    } catch {
      showNotice("Cannot connect to the server", "error");
    } finally {
      setBusyId(null);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-800 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="font-bold text-lg">Backup History</h2>
            <p className="text-sm text-gray-400 dark:text-gray-500">Backup files are stored in the database</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={handleBackup}
              disabled={isBackingUp}
              className={`px-4 py-2 rounded-xl font-semibold text-sm transition inline-flex items-center justify-center gap-2 ${
                isBackingUp
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg font-semibold text-xs bg-red-600 hover:bg-red-700 text-white transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={total === 0}
              className="px-3 py-2 rounded-lg font-semibold text-xs border border-red-600 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Delete All
            </button>
            <div className="relative">
              <input
                ...
                placeholder="Search file, creator, status..."
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-gray-400 dark:text-gray-500">Loading backups...</td>
                </tr>
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-gray-400 dark:text-gray-500">No backups yet. Click "Create Backup Now".</td>
                </tr>
              ) : (
                rows.map((b) => (
      ...
      <ConfirmDialog
        ...
        title={bulkDialog.type === "all" ? "Delete All Backups?" : "Delete Selected Backups?"}
        message={
          bulkDialog.type === "all"
            ? `This will delete all backup files (${total} total). This cannot be undone. Are you sure?`
            : `This will delete ${selected.size} selected backup file(s). This cannot be undone. Are you sure?`
        }
        confirmLabel="Delete"
        variant="danger"
        loading={bulkLoading}
  ...
  return (
    <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-3">
      <p className="text-sm text-gray-400 dark:text-gray-500">
        Showing <span className="font-semibold text-gray-600 dark:text-gray-300">{total}</span> data · page {page} of {totalPages}
      </p>
